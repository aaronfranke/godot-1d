...
Rect2 Sprite1D::_get_anchorable_rect() const {
	return get_rect();
}
...
void Sprite1D::item_rect_changed(bool p_size_changed) {
	if (p_size_changed) {
		queue_redraw();
	}
	emit_signal(StringName("item_rect_changed"));
}
...
void Sprite1D::set_texture(const Ref<Texture2D> &p_texture) {
	...
	if (_texture.is_valid()) {
		_texture->disconnect(StringName("changed"), Callable(this, StringName("_texture_changed")));
	}

	_texture = p_texture;

	if (_texture.is_valid()) {
		_texture->connect(StringName("changed"), Callable(this, StringName("_texture_changed")));
		_frame_count = _texture->get_height();
	}

	queue_redraw();
	_texture_changed();
	emit_signal(StringName("texture_changed"));
	item_rect_changed();
}
...
void Sprite1D::set_frame(const int p_frame) {
	...
	emit_signal(StringName("frame_changed"));
}
...
void Sprite1D::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("_texture_changed"), &Sprite1D::_texture_changed);
